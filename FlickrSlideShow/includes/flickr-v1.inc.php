...
function getFlickrPhotos($useProxy, $chosenTags)
{
	...
	*/
	...
	// Out of interest, lets echo out the completed URL
	echo $flickr_search . "<br/>";
	...
	// Just out of interest let's echo out the response XML
	echo $rsp->asXML();
	...
	foreach($rsp->photos->photo as $photo)
	{
		...
		foreach($sizes_rsp->sizes->size as $size)
		{
			...
			{
				// Get the "source" attribute of this <size> and build a img tag and echo it out
				echo '<img src="' . $size['source'] .'"/><br/>';
				
				// Get the "source" attribute of this <size> and build a link and echo it out
				echo '<a href="' . $size['source'] . '">' . $photo['title'] . '</a><br/>';
